...
from basicsr.data.data_util import (paired_paths_from_folder,
                                    ...
                                    Packing,
                                    Unpacking,
                                    paired_paths_from_folder_simple)
...
class PairedImageDataset(data.Dataset):
    """Paired image dataset for image restoration (new pairing rules)."""
    ...
    def __init__(self, opt):
        ...
        self.bgr2rgb = opt.get('bgr2rgb', True)

        self.gt_folder, self.lq_folder = opt['dataroot_gt'], opt['dataroot_lq']

        # 新增：控制配对规则
        # pairing_mode: "one_to_one"(默认) or "one_to_many"
        self.pairing_mode = opt.get('pairing_mode', 'one_to_one')
        self.multi_mode = opt.get('multi_mode', 'expand')               # expand/choose
        self.choose_policy = opt.get('choose_policy', 'sorted_first')   # sorted_first/random
        self.seed = opt.get('seed', 42)

        # 兼容旧配置：filename_tmpl 仍允许存在，但 folder 模式下不再依赖它
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            # meta_info_file 通常是严格列好的 (lq, gt) 对，不建议套用 ID 规则
            # 仍按原逻辑走
            ...
            # folder: 使用新规则
            one_to_many = (self.pairing_mode == 'one_to_many')
            self.paths = paired_paths_from_folder_simple(
                [self.lq_folder, self.gt_folder],
                ['lq', 'gt'],
    ...
    def __getitem__(self, index):
        if self.file_client is None:
            # 注意：这里 pop 会修改 dict，本来就是你原代码逻辑，我保持不动
