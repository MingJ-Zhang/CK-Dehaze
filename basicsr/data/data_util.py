...
def paired_paths_from_folder_simple(
    folders,
    keys,
    one_to_many=False,             # False: 一对一(同名)；True: 一对多(ID_*)
    multi_mode="expand",           # one_to_many=True 时生效: "expand" or "choose"
    choose_policy="sorted_first",  # "sorted_first" or "random"
    seed=42
):
    """Folder 配对（新规则）：
    - 一对一：lq 与 gt 同名（stem 相同）
    - 一对多：lq 为 ID_xxx，gt 为 ID；按 ID 匹配
    """
    assert len(folders) == 2
    assert len(keys) == 2
    input_folder, gt_folder = folders
    ...
    def full(folder, p):
        return p if osp.isabs(p) else osp.join(folder, osp.basename(p))
    ...
    # ---- 一对一：同名配对 ----
    if not one_to_many:
        gt_map = {stem(g): full(gt_folder, g) for g in gt_paths}
        ...
        for inp in input_paths:
            ...
            if s not in gt_map:
                continue
            paths.append({
                f"{input_key}_path": full(input_folder, inp),
                f"{gt_key}_path": gt_map[s]
            })
        return paths

    # ---- 一对多：ID_*.ext -> ID.ext ----
    gt_by_id = {stem(g).split('_')[0]: full(gt_folder, g) for g in gt_paths}

    haze_by_id = defaultdict(list)
    for inp in input_paths:
        hid = stem(inp).split('_')[0]
        haze_by_id[hid].append(full(input_folder, inp))

    # 保证可复现
    for hid in haze_by_id:
        haze_by_id[hid].sort()

    for hid, haze_list in haze_by_id.items():
        if hid not in gt_by_id:
            continue
        gt_full = gt_by_id[hid]

        if multi_mode == "expand":
            for hfull in haze_list:
                paths.append({
                    f"{input_key}_path": hfull,
                    f"{gt_key}_path": gt_full
                })
        elif multi_mode == "choose":
            if choose_policy == "random":
                hfull = rng.choice(haze_list)
            else:
                hfull = haze_list[0]
            paths.append({
                f"{input_key}_path": hfull,
