...
# Import KAM convolution (Chebyshev-based)
from .kam_conv import KAMConv2DLayer


def _resolve_kam_flag(use_kam, use_kan):
    return bool(use_kam) or bool(use_kan)
...
##########################################################################
## CK Attention Feed-Forward Network (CKAFFN)
class FeedForward(nn.Module):
    def __init__(self, dim, ffn_expansion_factor, bias, use_kan=False, use_kam=False):
        ...
        use_kam = _resolve_kam_flag(use_kam, use_kan)
        if use_kam:
            # Use KAM convolution for enhanced nonlinear feature extraction
            self.dwconv = KAMConv2DLayer(
                input_dim=hidden_features*2,
                output_dim=hidden_features*2,
                kernel_size=3,
                poly_order=3,
                groups=hidden_features*2,  # Maintain depthwise convolution
                padding=1,
                stride=1,
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias, use_kan=False, use_kam=False):
        ...
        use_kam = _resolve_kam_flag(use_kam, use_kan)
        if use_kam:
            # Use KAM convolution for QKV feature extraction
            self.qkv_dwconv = KAMConv2DLayer(
                input_dim=dim*3,
                output_dim=dim*3,
                kernel_size=3,
                poly_order=3,
                groups=dim*3,  # Maintain depthwise convolution
                padding=1,
                stride=1,
                base_activation=nn.GELU,
                dropout=0.0
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type,
        ...
        self.attn = Attention(dim, num_heads, bias, use_kam=use_kam)
        self.norm2 = LayerNorm(dim, LayerNorm_type)
        self.ffn = FeedForward(dim, ffn_expansion_factor, bias, use_kam=use_kam)
...
class SGCKUnet(nn.Module):
    def __init__(self, img_channel=3, width=64, middle_blk_num=1, 
                 ...
                 middle_patch_size=8,
                 use_kan=False,
                 use_kam=False):  # Add KAM switch for ablation study
        super().__init__()
        ...
        if use_kam:
            print("=" * 60)
            print("[SG-CK-Unet] KAM Convolution ENABLED")
            print("  - FeedForward dwconv: Standard Conv -> KAM Conv")
            print("  - Attention qkv_dwconv: Standard Conv -> KAM Conv")
            print("=" * 60)
        else:
            print("[SG-SAS-Unet] Using standard convolutions")
        ...
        for i, num in enumerate(enc_blk_nums):
            self.encoders.append(nn.Sequential(*[TransformerBlock(dim=chan, num_heads=restormer_heads[i], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, use_kam=use_kam) for _ in range(num)]))
            self.downs.append(nn.Conv2d(chan, 2*chan, 2, 2))
            chan *= 2
            
        self.middle_blks = nn.Sequential(*[TransformerBlock(dim=chan, num_heads=restormer_middle_heads, ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, use_kam=use_kam) for _ in range(middle_blk_num)])
        ...
        for i, num in enumerate(dec_blk_nums):
            ...
            self.decoders.append(nn.Sequential(*[TransformerBlock(dim=chan, num_heads=current_restormer_heads, ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, use_kam=use_kam) for _ in range(num)]))
...
class CKUnet(nn.Module):
    def __init__(self, img_channel=3, width=64, middle_blk_num=1, 
        ...
        if use_kam:
            print("=" * 60)
            print("[CK-Unet] KAM Convolution ENABLED")
            print("  - FeedForward dwconv: Standard Conv -> KAM Conv")
            print("  - Attention qkv_dwconv: Standard Conv -> KAM Conv")
            print("=" * 60)
        else:
            print("[CK-Unet] Using standard convolutions")
