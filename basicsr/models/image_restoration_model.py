...
class ImageRestorationModel(BaseModel):
    ...
    def setup_optimizers(self):
        ...
        if optim_type == 'Adam':
            ...
            from basicsr.models.optimizers import SingleDeviceMuon
            self.optimizer_g = SingleDeviceMuon(optim_params, **optim_config)
        elif optim_type == 'MuonWithAuxAdam':
            from basicsr.models.optimizers import SingleDeviceMuonWithAuxAdam
            muon_params = []
            adam_params = []
            ...
            muon_hparams = optim_config.get('muon_hparams', {'lr': 0.02, 'momentum': 0.95, 'weight_decay': 0.0})
            adam_hparams = optim_config.get('adam_hparams', {'lr': 3e-4, 'betas': (0.9, 0.95), 'eps': 1e-10, 'weight_decay': 0.0})
            ...
            self.optimizer_g = SingleDeviceMuonWithAuxAdam(param_groups_for_muon)
            ...
            if use_align:
                ...
                if hasattr(self, 'gt'):
                    try:
                        ...
                        sr_img_to_save = cv2.cvtColor(aligned_sr_np, cv2.COLOR_RGB2BGR)
                        gt_img_to_save = cv2.imread(val_data['gt_path'][0], cv2.IMREAD_COLOR)

                    except Exception as e:
                        if rank == 0:
                            logger.error(f"Alignment or metric calculation failed for {img_name}: {e}. Skipping metrics for this image.")

                        sr_img_to_save = tensor2img(self.output, rgb2bgr=True, out_type=np.float32)
                        gt_img_to_save = cv2.imread(val_data['gt_path'][0], cv2.IMREAD_COLOR) if hasattr(self, 'gt') else None
                else:
                    sr_img_to_save = tensor2img(self.output, rgb2bgr=True, out_type=np.float32)

                if save_img:
                    if sr_img_to_save is not None:
                        imwrite((sr_img_to_save * 255.).clip(0, 255).astype(np.uint8), self.get_save_path('sr', img_name, current_iter, dataset_name))
                    if gt_img_to_save is not None:
                        imwrite(gt_img_to_save, self.get_save_path('gt', img_name, current_iter, dataset_name))
                ...
                if save_img:
                    if sr_img.shape[2] == 6:
                        ...
                        if 'gt' in visuals:
                            imwrite(cv2.imread(val_data['gt_path'][0], cv2.IMREAD_COLOR), save_gt_img_path)
